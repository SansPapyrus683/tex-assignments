...
I've only used my own brain + posted slides/resources.
...
\begin{align*}
  H(X)
   ...
   & = \boxed{\frac{(p-1)\log(1-p)}{p}-\log p}
\end{align*}
...
Plotting it on MATLAB gives the following (sorry idk what's up with the resolution):
\begin{center}
  \includegraphics[width=10cm]{img/hw1/p_diff_q}
\end{center}
...
\begin{center}
  \includegraphics[width=10cm]{img/hw1/q_diff_p}
\end{center}
...
\begin{align*}
  I(P_W; W)
   & = H(W) - H(W \mid P_W)                                                                                   \\
   & = -(0.1\log 0.1 + 0.9\log 0.9) + \left(1 + 0.15 \cdot \log\frac{3}{5} + 0.1 \cdot \log\frac{2}{5}\right) \\
   & \approx \boxed{0.288}
\end{align*}
...
However, since conditioning can only decrease entropy,
\[H(B_i' \mid B_1', \cdots, B_{i-1}') \le H(B_i') = H(B_i)\]
so entropy of drawing balls without replacement is \textit{at most}
the entropy of drawing balls with.

% To check if this inequality is strict, lemme evaluate $H(B_2' \mid B_1')$:
