...
\begin{gather*}
    P(X=1 \mid \hat{X}=0)=\frac{2D(p-D)}{1-2D} \\
    P(X=1 \mid \hat{X}=1)=\frac{2(1-D)(1-p-D)}{1-2D} \\
    P(X=0 \mid \hat{X}=1)=\frac{2D(1-p-D)}{1-2D}
\end{gather*}
...
We have $\hat{X}=X-Z=X+Z$ since $Z$ has zero mean, so once we know $x$, $\hat{X} \mid x \sim \mathcal{N}(x, D)$.
...
\begin{align*}
    ...
    &= \E\left[X^2(a^2+1-2a)+2a^2XZ+a^2Z^2-2aZ\right] \\
    &= (a-1)^2\E\left[X^2\right]+2a^2\E[X]\E[Z]+a^2\E\left[Z^2\right]-2a\E[Z] \\
