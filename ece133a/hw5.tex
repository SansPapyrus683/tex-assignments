...
\begin{align*}
     ...
     &= a^T(I-QQ^T)a
\end{align*}
where $QQ^T$ a matrix that's all zeros except the first $n$ elements on the diagonal.

This means that $(I-QQ^T)a$ is just the first $n$ elements of $a$,
and we can compute the denominator in just $n$ flops.

It doesn't cost too much to put these elements together, since it's all just elementwise stuff at this point.
This gives us a flop count of (again) \boxed{2mn+n^2} to compute $\hat{y}$ and $\hat{z}$.
